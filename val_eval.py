# ============================================================
# val_eval.py — GREEDY GLOBAL BEST (DISK = TRUTH)
# ============================================================
...
PRED_IMG_DIR = os.path.join(eval_root_val, "pred-imgs")
PRED_CSV_DIR = os.path.join(eval_root_val, "pred-csv")
EVAL_STATUS_FILE = os.path.join(eval_root_val, "eval_status.csv")
...
def main():
    ...
    seeding(seed_val)
    append_run_log("VAL_EVAL", "Validation evaluation started")

    # Prepare directories
    prepare_eval_dirs(eval_root_val)
    ...
    # Dataset (VALIDATION)
    val_x = sorted(glob(main_dataset_dir + val_images_dir))
    val_y = sorted(glob(main_dataset_dir + val_masks_dir))
    ...
    # --------------------------------------------------------
    # CHECKPOINTS (GREEDY: ONLY EXISTING, SKIP LOCKED)
    # --------------------------------------------------------
    ...
    for ckpt_path in checkpoints:
        ...
        # Skip immutable best checkpoints
        if "_best_iteration_" in ckpt_name:
            continue

        # Skip if deleted before evaluation
        ...
        append_run_log("VAL_EVAL", f"Evaluating {ckpt_name}")
        ...
        # Skip CSV write if checkpoint vanished mid-run
        if not os.path.exists(ckpt_path):
            continue
    ...
    append_run_log("VAL_EVAL", "Validation evaluation completed")
